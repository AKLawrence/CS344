...
*  otp_enc doesn't do the encryption, otp_end_d does. OTP_DEC_D will run in the background, as a daemon and will take in a port
...
int main(int argc, char *argv[])
{
	...
	int pid;
	int shift_cipher, shift_key, difference, decrypted_output;
	...
	while(1) {
		...
		if (establishedConnectionFD < 0) {
			fprintf(stderr, "OTP_DEC_D: ERROR on accepting socket connection\n");
		}
		...
		if (pid == 0){
			...
			// Get the cipher file data message from the client and display it
			memset(buffer_cipher, '\0', BUFFER_SIZE);
			charsRead = recv(establishedConnectionFD, buffer_cipher, BUFFER_SIZE, 0); // Read the client's message from the socket
			if (charsRead < 0) {
				fprintf(stderr, "OTP_DEC_D: ERROR reading cipher input from socket\n");
			}


			// Send a Success message back to the client
			charsRead = send(establishedConnectionFD, "I am the server, and I got your cipher data message", 54, 0); // Send success back
			if (charsRead < 0) {
			...
			if (charsRead2 < 0) {
				fprintf(stderr, "OTP_DEC_D: ERROR reading key data from socket\n");
			}


			// Begin ciphering text based on plaintext buffer_cipher data and buffer_key data.
			// Shift each character in buffer_cipher so their ASCII decimal values are instead between 0 and 26, including swapping ' ' to '@' first. 

			for (int k = 0; k < (strlen(buffer_cipher)-1); k++){
				decrypted_output = 0;
				if (buffer_cipher[k] == ' '){
					buffer_cipher[k] = '@';
				}
				...
				shift_cipher = (int)buffer_cipher[k];
				shift_key = (int)buffer_key[k];
				shift_cipher -= 64;
				shift_key -= 64;
				difference = (shift_cipher - shift_key);	// subtracting these values, to set us up for our modulus operation that accounts for negative values
				if (difference < 0){
					difference += 27;
				}
				decrypted_output = difference + 64;
				if ( (char)decrypted_output == '@' ) {
					strcpy(&buffer_output[k], " ");
				} else {
					buffer_output[k] = (char) decrypted_output;
				}
			}
